...
local function on_attach(ev)
    ...
    map(
        'n',
        ']d',
        function() vim.diagnostic.goto_next({ severity = nil }) end,
        { desc = 'Next diagnostic' }
    )
    map(
        'n',
        '[d',
        function() vim.diagnostic.goto_prev({ severity = nil }) end,
    ...
    map(
        ...
        { desc = 'Search diagnostics (all buffers)' }
    )
...
return {
    ...
    init = function()
        ...
        vim.lsp.handlers['textDocument/hover'] =
            vim.lsp.with(vim.lsp.handlers.hover, {}) --, { max_width = max_width })
        vim.lsp.handlers['textDocument/signatureHelp'] =
            vim.lsp.with(vim.lsp.handlers.signature_help, {}) --, { max_width = max_width })
